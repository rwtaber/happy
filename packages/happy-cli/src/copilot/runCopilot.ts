...
import { isCommandAvailable } from '@/utils/detectCLI';
 ...
 *
 * Uses the same PATH probe as CLI availability detection so the launch guard
 * and the app's agent picker never disagree about whether Copilot is present.
 */
...
export async function runCopilot(opts: {
    ...
    // Share the ACP command mapping with `happy acp copilot` so the two entry
    // points cannot drift apart on the binary or its flags.
    const { command, args } = KNOWN_ACP_AGENTS.copilot;
